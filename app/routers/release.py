...
# Get all releases
@router.get("/release", status_code=status.HTTP_200_OK, response_model=List[Return])
def get_releases(db: Session = Depends(get_db)):
    releases = db.query(models.Release).all()
    return releases

# Add a release
@router.post("/release", status_code=status.HTTP_201_CREATED, response_model=Return)
...
@router.get("/release/{id}", status_code=status.HTTP_200_OK, response_model=Return)
...
@router.delete("/release/{id}", status_code=status.HTTP_204_NO_CONTENT)
...
@router.put("/release/{id}", response_model=Return)
